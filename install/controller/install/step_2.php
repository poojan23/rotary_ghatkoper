...
class ControllerInstallStep2 extends PT_Controller
{
    private $error = array();
    ...
    public function index()
    {
        ...
        $data['template_config'] = DIR_POPAYA . 'config.php';
        $data['admin_config'] = DIR_POPAYA . 'admin/config.php';
        $data['sadmin_config'] = DIR_POPAYA . 'sadmin/config.php';

        # template config
        if (!is_file(DIR_POPAYA . 'config.php')) {
            $data['template_config_err'] = $this->language->get('text_missing');
        } elseif (!is_writable(DIR_POPAYA . 'config.php')) {
            $data['template_config_err'] = $this->language->get('text_unwritable');
        } else {
            $data['template_config_err'] = '';
        }

        # admin configs
        if (!is_file(DIR_POPAYA . 'admin/config.php')) {
            $data['admin_config_err'] = $this->language->get('text_missing');
        } elseif (!is_writable(DIR_POPAYA . 'admin/config.php')) {
            $data['admin_config_err'] = $this->language->get('text_unwritable');
        } else {
            $data['admin_config_err'] = '';
        }

        # sadmin configs
        if (!is_file(DIR_POPAYA . 'sadmin/config.php')) {
            $data['sadmin_config_err'] = $this->language->get('text_missing');
        } elseif (!is_writable(DIR_POPAYA . 'admin/config.php')) {
            $data['sadmin_config_err'] = $this->language->get('text_unwritable');
        } else {
            $data['sadmin_config_err'] = '';
        }
    ...
    private function validate()
    {
        ...
        } elseif (!is_file(DIR_POPAYA . 'admin/config.php')) {
            $this->error['warning'] = $this->language->get('error_admin_exist');
        } elseif (!is_writable(DIR_POPAYA . 'admin/config.php')) {
            $this->error['warning'] = $this->language->get('error_admin_writable');
        } elseif (!is_file(DIR_POPAYA . 'sadmin/config.php')) {
            $this->error['warning'] = $this->language->get('error_admin_exist');
        } elseif (!is_writable(DIR_POPAYA . 'sadmin/config.php')) {
            $this->error['warning'] = $this->language->get('error_admin_writable');
        }
