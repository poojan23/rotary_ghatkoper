...
class ModelCatalogEvent extends PT_Model {
    ...
    public function getEvent($event_id) {
        $query = $this->db->query("SELECT e.date,e.event_url,e.name,e.description,e.image as event_image,e.sort_order as e_order,e.status,ei.* FROM " . DB_PREFIX . "events e LEFT JOIN " . DB_PREFIX . "event_image ei ON (e.event_id = ei.event_id)"
    ...
    public function get_Events() {
        ...
        $query = $this->db->query("SELECT e.date,e.event_url,e.name,e.description,e.image as event_image,e.sort_order as e_order,e.status,ei.* FROM " . DB_PREFIX . "events e LEFT JOIN " . DB_PREFIX . "event_image ei ON (e.event_id = ei.event_id)"
                . "  WHERE (e.date BETWEEN '" . $curr_year . "' AND '" . $nxt_year . "' )");

        return $query->rows;
    }
    
    public function getPrvEvents() {
        $prv_year = date("Y",strtotime('-1 year'));
        $curr_year = date("Y");
        $query = $this->db->query("SELECT e.date,e.event_url,e.name,e.description,e.image as event_image,e.sort_order as e_order,e.status,ei.* FROM " . DB_PREFIX . "events e LEFT JOIN " . DB_PREFIX . "event_image ei ON (e.event_id = ei.event_id)"
                . "  WHERE (e.date BETWEEN '" . $prv_year . "' AND '" . $curr_year . "' )");
