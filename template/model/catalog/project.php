...
class ModelCatalogProject extends PT_Model {
    ...
    public function getProject($project_id) {
        $query = $this->db->query("SELECT e.date,e.name,e.project_url,e.description,e.sort_order as e_order,e.status,e.image as project_image,ei.* FROM " . DB_PREFIX . "projects e LEFT JOIN " . DB_PREFIX . "project_image ei ON (e.project_id = ei.project_id) "
                . "WHERE e.project_id = '" . (int) $project_id . "'");
    ...
    public function getProjects() {
        ...
        $query = $this->db->query("SELECT e.date,e.name,e.project_url,e.description,e.sort_order as e_order,e.status,e.image as project_image,ei.* FROM " . DB_PREFIX . "projects e "
                . "LEFT JOIN " . DB_PREFIX . "project_image ei ON (e.project_id = ei.project_id) WHERE (e.date BETWEEN '" . $curr_year . "' AND '" . $nxt_year . "' )");

        return $query->rows;
    }

    public function getPrvProjects() {
        $prv_year = date("Y",strtotime('-1 year'));
        $curr_year = date("Y");
        $query = $this->db->query("SELECT e.date,e.name,e.project_url,e.description,e.sort_order as e_order,e.status,e.image as project_image,ei.* FROM " . DB_PREFIX . "projects e "
                . "LEFT JOIN " . DB_PREFIX . "project_image ei ON (e.project_id = ei.project_id) WHERE (e.date BETWEEN '" . $prv_year . "' AND '" . $curr_year . "' )");
